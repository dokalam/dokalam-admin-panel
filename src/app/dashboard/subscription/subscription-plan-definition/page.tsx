...
const Page = () => {
  ...
  const registerAndConfirm = ()=>{
    if(title.length == 0 || duration.length == 0 || price.length == 0 || iconImage == null || productId.length == 0 ){
  ...
  const checkedAndRegister = async()=>{
    ...
    let data = {
      query: `
          mutation newSubscriptionPlanDefinition(
            $product_id : String!,
            $title : String,
            $description : String,
            $badg : String,
    ...
    await axios({
        ...
        setLoading(false);
        if (response.data?.data?.newSubscriptionPlanDefinition?.status == 200) {
            toast.success(response.data?.data?.newSubscriptionPlanDefinition?.message, {
              position: "top-center",
              autoClose: 3000,
              hideProgressBar: false,
              closeOnClick: true,
    ...
    <div className="flex flex-col justify-between w-full lg:w-[600px] 2xl:w-[750px] mt-10 mb-28 px-4 sm:mx-auto">
      ...
      <div className="mt-6">
        ...
        >
          ...
          <span className="text-red-500 px-1">*</span>
          <div className={`mt-1 flex gap-2 w-full items-center justify-between`}>
            <Input type="number" id="number-stage-season" value={duration} changeState={setDuration} classes="flex-1" inputStyles="!text-base" />
          </div>
        </label>
      </div>
      <div className="mt-6">
