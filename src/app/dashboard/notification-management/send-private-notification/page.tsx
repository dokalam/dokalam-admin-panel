...
const Page = () => {
  ...
  const checkedAndRegister = async () => {
    ...
    let data = {
      query: `
          mutation registerNewPrivateNotificationByAdmin(
            $user : ID!,
            $title : String!,
            $body : String!,
            $link : String,
            ...
            registerNewPrivateNotificationByAdmin(
              user : $user,
              title : $title,
              body : $body,
              link : $link,
    ...
    await axios({
      ...
      .then(async (response) => {
        setLoading(false);
        if (response.data?.data?.registerNewPrivateNotificationByAdmin?.status == 200) {
            toast.success(response.data?.data?.registerNewPrivateNotificationByAdmin?.message, {
              position: "top-center",
              autoClose: 3000,
              hideProgressBar: false,
              closeOnClick: true,
...
pointer-events-none inline-block h-[22px] w-[22px] transform rounded-full shadow-lg ring-0 transition duration-200 ease-in-out`}
      ...
      <Footer buttonFn={registerAndConfirm} buttonText="ارسال اعلان خصوصی" loadingButton={loading} classes="md:!mr-72 !justify-end" />
      <PackageList
        ref={(Ref) => {
