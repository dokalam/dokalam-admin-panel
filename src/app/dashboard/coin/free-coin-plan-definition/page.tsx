...
const FreeCoinType:SelectedOption[] = [
  {value:null, label:"انتخاب نوع آیتم سکه رایگان"},
...
const Page = () => {
  ...
  const [freeCoinType, setFreeCoinType] = useState<string | null>(null)
  ...
  const registerAndConfirm = ()=>{
    if(freeCoinType == "private" && (!userSelected || userSelected?.length < 1)){
    ...
    } else if(title.length == 0 || numberCoin.length == 0 || !freeCoinType ){
  ...
  const checkedAndRegister = async()=>{
    ...
    let data = {
      query: `
          mutation newFreeCoinPlanDefinition(
            $type : String!,
            $private_users : [ID],
            $title : String!,
            $description : String,
      ...
      variables: {
        type : freeCoinType,
        private_users : freeCoinType == "private"? userSelected?.map((item:any)=>item._id):undefined,
        title : title,
    ...
    await axios({
        ...
        setLoading(false);
        if (response.data?.data?.newFreeCoinPlanDefinition?.status == 200) {
            toast.success(response.data?.data?.newFreeCoinPlanDefinition?.message, {
              position: "top-center",
              autoClose: 3000,
              hideProgressBar: false,
              closeOnClick: true,
  ...
  return (
    <div className="flex flex-col justify-between w-full lg:w-[600px] 2xl:w-[750px] mt-10 mb-28 px-4 sm:mx-auto">
      ...
      <div className="mt-6">
        ...
        >
          ...
          <div className={`mt-1 flex-1 gap-2 w-full items-center justify-between`}>
            <SelectInput
              name="free-coin-type"
              options={FreeCoinType}
              onChange={(value) => setFreeCoinType(value)}
            />
          </div>
        </label>
      </div>
      ...
      {(userSelected && userSelected.length > 0 && freeCoinType !== "public")&&(
      ...
      <div className="mt-6">
        ...
        >
          ...
          <div className={`mt-1 flex gap-2 w-full items-center justify-between`}>
            <Input type="number" id="number-coin" value={numberCoin} changeState={setNumberCoin} classes="flex-1" inputStyles="!text-base" />
          </div>
        </label>
      </div>
      <div className="mt-6">
