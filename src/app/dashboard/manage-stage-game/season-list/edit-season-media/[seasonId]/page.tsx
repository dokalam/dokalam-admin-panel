...
const Page = () => {
  ...
  const deletePreviousMedia = async ({path, type}:{path:string, type:string}) => {
    let data = {
      query: `
          mutation deleteMediaFromStageGameSeason(
            $_id : ID!,
            $path : String!,
            $type : String!,
            $change_version_updated : Boolean!
          ){
            deleteMediaFromStageGameSeason(
              _id : $_id,
              path : $path,
              type : $type,
    ...
    await axios({
      ...
      .then(async (response) => {
        const res = response.data?.data?.deleteMediaFromStageGameSeason
        if (res?.status == 200) {
  ...
  const checkedAndRegister = async()=>{
    ...
    await axios({
      ...
      .then(async (response) => {
        setLoading(false);
        if (response.data?.data?.addNewMediaToStageGameSeason?.status == 200) {
            toast.success(response.data?.data?.addNewMediaToStageGameSeason?.message, {
              position: "top-center",
              autoClose: 3000,
              hideProgressBar: false,
              closeOnClick: true,
