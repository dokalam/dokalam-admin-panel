...
const Page = () => {
  ...
  const deletePreviousMedia = async ({path, type}:{path:string, type:string}) => {
    let data = {
      query: `
          mutation deleteMediaFromStageGameStage(
            $_id : ID!,
            $path : String!,
            $type : String!,
            $change_version_updated : Boolean!
          ){
            deleteMediaFromStageGameStage(
              _id : $_id,
              path : $path,
              type : $type,
    ...
    await axios({
      ...
      .then(async (response) => {
        const res = response.data?.data?.deleteMediaFromStageGameStage
        if (res?.status == 200) {
  ...
  const checkedAndRegister = async()=>{
    ...
    let queryData = {
      query: `
          mutation addNewMediaToStageGameStage(
            $_id : ID!,
            $media : [FileInput],
            $voice : [FileInput],
            $change_version_updated : Boolean!
          ){
            addNewMediaToStageGameStage(
              _id : $_id,
              media : $media,
              voice : $voice,
              change_version_updated : $change_version_updated,
    ...
    await axios({
      ...
      .then(async (response) => {
        setLoading(false);
        if (response.data?.data?.addNewMediaToStageGameStage?.status == 200) {
            toast.success(response.data?.data?.addNewMediaToStageGameStage?.message, {
              position: "top-center",
              autoClose: 3000,
              hideProgressBar: false,
              closeOnClick: true,
  ...
  return (
    ...
    <div className="flex flex-col justify-between w-full lg:w-[600px] 2xl:w-[750px] mt-10 mb-28 px-4 sm:mx-auto">
      <p className="text-justify font-['iransans-md'] text-primary text-[16px] mb-2">
        <span className="text-text5 dark:text-text5_dark">مرحله</span>
        {` ${data?.stage_number_in_language} `}
