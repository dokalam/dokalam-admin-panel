...
const Page = () => {
  ...
  const deletePreviousMedia = async ({path, type}:{path:string, type:string}) => {
    let data = {
      query: `
          mutation deleteMediaFromPackageGameSeason(
            $_id : ID!,
            $path : String!,
            $type : String!,
            $change_version_updated : Boolean!
          ){
            deleteMediaFromPackageGameSeason(
              _id : $_id,
              path : $path,
              type : $type,
    ...
    await axios({
      ...
      .then(async (response) => {
        const res = response.data?.data?.deleteMediaFromPackageGameSeason
        if (res?.status == 200) {
  ...
  const checkedAndRegister = async()=>{
    ...
    let queryData = {
      query: `
          mutation addNewMediaToPackageGameSeason(
            $_id : ID!,
            $media : [FileInput],
            $music : FileInput,
            $change_version_updated : Boolean!
          ){
            addNewMediaToPackageGameSeason(
              _id : $_id,
              media : $media,
              music : $music,
              change_version_updated : $change_version_updated,
      ...
      variables: {
        ...
        media: media.map((item:any, index:number) => ({
          file: null,
          order: (index+1 + data?.media?.length),
    ...
    await axios({
      ...
      .then(async (response) => {
        setLoading(false);
        if (response.data?.data?.addNewMediaToPackageGameSeason?.status == 200) {
            toast.success(response.data?.data?.addNewMediaToPackageGameSeason?.message, {
              position: "top-center",
              autoClose: 3000,
              hideProgressBar: false,
              closeOnClick: true,
  ...
  return (
    ...
    <div className="flex flex-col justify-between w-full lg:w-[600px] 2xl:w-[750px] mt-10 mb-28 px-4 sm:mx-auto">
      <div className="flex w-full items-center justify-between mt-6">
        {
          data?.package_info?.icon_image && (
            <ImageComponent
              parentclasses="w-16 h-16 lg:h-[60px] lg:w-[60px] 2xl:h-[60px] 2xl:w-[60px] !rounded-xl"
      ...
      <p className="text-justify font-['iransans-md'] text-primary text-[16px] mb-2 mt-6">
        <span className="text-text5 dark:text-text5_dark">فصل</span>
        {` ${data?.title} `}
      </p>
      {(data?.media.length > 0 || data?.music)&& (
              ...
              <div key={`${index.toString()}`} className="w-full h-20 3xs:h-24 sm:h-28 flex justify-center items-center">
                ...
                >
                  ...
                  <div className="absolute top-0 w-full flex justify-between px-1 pt-1">
                    <div
                      className={`flex justify-center items-center rounded transition text-primary bg-[#00000099] sm:hover:bg-[#33333370] text-lg w-6 h-6`}
                    >
