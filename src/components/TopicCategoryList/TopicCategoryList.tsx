...
import { TopicCategoryListModalInterface } from "@/interfaces/ModalInterface";
import { toast } from "react-toastify";
import moment from "moment-jalaali";
import "moment/locale/fa";
import TopicCategorySelectItem from "./TopicCategorySelectItem";
moment.loadPersian({ usePersianDigits: false, dialect: "persian-modern" });

const TopicCategoryList = forwardRef((_, ref) => {
  ...
  const openModal = (options: TopicCategoryListModalInterface) => {
  ...
  const getDataForFirst = async () => {
    await axios({
      ...
      data: {
        query: `
            query getAllTopicCategoryForAdmin($page : Int, $limit : Int, $filter_parent : ID){
                getAllTopicCategoryForAdmin(page : $page, limit : $limit, filter_parent : $filter_parent) {
                    list{
                        _id,
                        title,
                        child,
                        image,
                        sub_category{_id, title, child, image, sub_category{_id, title, child, image, sub_category{_id, title, child, image}}}
                    },
                    hasNextPage,
                    nextPage
                }
      ...
      .then((response) => {
        const riciveData = response.data.data.getAllTopicCategoryForAdmin;
  ...
  const deletedSubItem = ({ sub }: { sub: any }) => {
    const index = extendedState._id.findIndex((i: any) => i == sub._id);
    extendedState._id.splice(index, 1);
    extendedState.title.splice(index, 1);
    extendedState.image.splice(index, 1)
    setExtendedState({ ...extendedState });
  ...
  const selectedSubItem = ({ sub }: { sub: any }) => {
    if (numberSelected == 1) {
      const _id = sub._id;
                              ...
                              >
                                <ul role="list" className={`flex flex-col`}>
                                  {data?.map((item: any, index: number) => (
                                    <div
                                      className="px-4 transition cursor-pointer"
                                      key={index.toString()}
                                    >
                                      <TopicCategorySelectItem
                                        ...
                                        deletedSubItem={deletedSubItem}
                                        selectedSubItem={selectedSubItem}
                                        imageSrc={item?.image??null}
                                        child={item.child}
                                        subCategory={item.sub_category}
                                        extendedState={extendedState._id}
                                      />
                                      <Border />
                                    </div>
                                  ))}
...
TopicCategoryList.displayName = "TopicCategoryList";
