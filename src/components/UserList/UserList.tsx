...
const UserList = forwardRef((_, ref) => {
  ...
  const openModal = (options: UserListModalInterface) => {
    ...
    if (options.previousSelected) {
      setExtendedState({
        ...
        user_name: options?.previousSelected.user_name,
        phone: options?.previousSelected?.phone,
  ...
  const getDataForFirst = async (txt: any) => {
    await axios({
      ...
      data: {
        query: `
            query getAllUserForAdmin($page : Int, $limit : Int, $search : String){
                getAllUserForAdmin(page : $page, limit : $limit, search : $search) {
                    list{
                      _id,
                      type,
                      phone,
  ...
  const deletedItem = ({ item }: { item: any }) => {
    ...
    extendedState.user_name.splice(index, 1);
    extendedState.phone.splice(index, 1)
  ...
  const selectedItem = ({ item }: { item: any }) => {
    if (numberSelected == 1) {
      ...
      const name = (item?.first_name || item?.last_name)?`${item?.first_name} ${item?.last_name}`:null
      setExtendedState({
        _id: [_id],
        user_name: [user_name],
        phone: [phone],
      ...
      if (extendedState._id.length < numberSelected) {
        ...
        extendedState.user_name.push(user_name);
        extendedState.phone.push(phone)
                        ...
                        <div className={`${extendedState?._id?.length > 0 ? "h-[146px]" : "h-[106px]"}`}>
                          ...
                          >
                            ...
                            {extendedState?._id?.length > 0 && (
                              ...
                              >
                                {extendedState.user_name.map((item: any, index: number) => (
                                  <div
                                  ...
                                  >
                                    <p className="font-['iransans-md'] text-text dark:text-text_dark line-clmp-1">
                                      {item?`${item}`:""}
                                      {extendedState.phone[index]?<span className="text-warning"> | </span>:""}
                                      {extendedState.phone[index]?`${extendedState.phone[index]}`:""}
                                      {extendedState.name[index]?<span className="text-warning"> | </span>:""}
                                      {extendedState.name[index]?`${extendedState.name[index]}`:""}
                                    </p>
                                    <div
                                  ...
                                  {data?.map((item: any, index: number) => (
                                    ...
                                    >
                                      <UserSelectItem
                                        ...
                                        name={(item?.first_name || item?.last_name)?`${item?.first_name} ${item?.last_name}`:null}
